...
Imagem obterMatrizImagem2() {
	int i, j, ch, rowstride;
	guchar ***m, *pixels, *p;
	Imagem img2, temp2;
	
	GdkPixbuf *buffer = gtk_image_get_pixbuf(GTK_IMAGE(image2));
	img2.h = gdk_pixbuf_get_height(buffer);
	img2.w = gdk_pixbuf_get_width(buffer);
	img2.numCanais = gdk_pixbuf_get_n_channels(buffer);
	rowstride = gdk_pixbuf_get_rowstride(buffer);
	pixels = gdk_pixbuf_get_pixels(buffer);
	
	//Checa se o tamanho da segunda imagem difere do da primeira
	//if(img2.h > img.h || img2.w > img.w){
		/*temp2.h = img.h;
		temp2.w = img.w;
		temp2.numCanais = img.numCanais;
		for(i = 0; i<temp2.h; i++){
			for(j = 0; j<temp2.w; j++){
...
void carregarSegundaImagem(GtkWidget *widget, gpointer data) {
	...
	if(gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		nomeArquivo2 = gtk_file_chooser_get_filename( GTK_FILE_CHOOSER(dialog) );
		gtk_widget_destroy(dialog);
	}
	gtk_image_set_from_file(GTK_IMAGE(image2), nomeArquivo2);

	gtk_widget_queue_draw(image2);

	gtk_label_set_text(GTK_LABEL(label1), "Imagem carregada");
	original2 = obterMatrizImagem2();
}
...
void funcaoAplicar(GtkWidget *widget, gpointer data) {
	...
	Imagem res = meuFiltro(original, original2);
...
int main(int argc, char **argv) {
	...
	//cria dois botoes para carregar as imagens
	GtkWidget *botaoCarregar = gtk_button_new_with_label("Carregar Primeira Imagem");

	GtkWidget *botaoCarregar2 = gtk_button_new_with_label("Carregar Segunda Imagem");
	...
	//altera o tamanho dos botoes carregar
	gtk_widget_set_size_request(botaoCarregar, 70, 30);
	gtk_widget_set_size_request(botaoCarregar2, 70, 30);
	...
	gtk_container_add(GTK_CONTAINER(hbox), botaoCarregar2);
	...
	//conecta o evento clicked do botaoCarregar2 a funcao carregarSegundaImagem
	g_signal_connect(G_OBJECT(botaoCarregar2), "clicked", G_CALLBACK(carregarSegundaImagem), NULL);
